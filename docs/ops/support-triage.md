...
- check whether the Google Sheets append path is configured
- check whether the payment metadata fields are present in the payload path

### Report generation issues

- check backend logs for `dashboard-trigger`
- check whether Azure Blob and Chrome / Chromium configuration is present
- check whether the Google Sheets row was updated with `Final PDF link`

### Report delivery issues

- check backend logs for `dashboard-delivery`
- check whether `Final PDF link` and recipient email are present
- check whether the Google Sheets row was updated with delivery fields
