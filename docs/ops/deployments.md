...
| --- | --- | --- | --- |
| `LotLogic-Fe` | Push to `stage` or manual workflow dispatch | Azure Storage account `lotcheck` | Automated deployment is via Azure Storage; legacy AWS deploy scripts still exist in `package.json` |
| `LotLogic-Be` | Push to `stage` or manual workflow dispatch | Azure Container App `lotcheck-be` in resource group `Production` | Builds Docker image and updates the running app |
| `LotLogic-Free-Assessment` | Push to `main` or manual workflow dispatch | Azure Storage account `bpassessmentprod` | Public assessment app |
...
- The frontend and backend deploy from `stage`, while the public assessment app deploys from `main`.
