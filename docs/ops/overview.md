...
| `LotLogic-Be` | Shared backend and ops engine | PostgreSQL/PostGIS, Stripe, Google Maps, monday.com, Azure Blob, Mailgun SMTP, Entra ID |
...
Public user
  ...
  -> Analyst reviews and enriches monday.com item
  -> monday.com webhook -> LotLogic-Be /api/monday/dashboard-trigger
  -> PDF render + Azure Blob upload
  -> monday.com item updated with Final PDF link
  -> Operator marks Delivery status in monday.com
  -> monday.com webhook -> LotLogic-Be /api/monday/dashboard-delivery
...
| Azure Database for PostgreSQL flexible server / PostGIS | Backend data | Daily backups with 35-day retention |
| Stripe | Checkout and payment events | Source of truth for payment success |
| monday.com | Paid-report queue and report-ops workflow | Backend upserts paid-report items and reads item data for generation/delivery |
| Azure Blob Storage | PDF storage and admin uploads | Required for final report links |
| Mailgun SMTP | Customer and internal emails | Separate Mailgun SMTP accounts are used for LotCheck and BlockPlanner. LotCheck sends via `mail@mail.lotcheck.com.au`; BlockPlanner sends via `mail@mail.blockplanner.com.au`. |
...
| --- | --- | --- |
| `LotLogic-Fe` | `main` | `.github/workflows/deploy-azure-storage.yml` |
| `LotLogic-Be` | `main` | `.github/workflows/deploy-azure-acr-prod.yml` |
...
- The monday.com board structure and automation setup are operational dependencies outside these repos.
- Production URLs: LotCheck FE `https://demo.lotcheck.com.au/`, Free Assessment `https://discover.blockplanner.com.au/`, backend `https://lotcheck-be.wittysky-d6d60dbd.australiasoutheast.azurecontainerapps.io`.
